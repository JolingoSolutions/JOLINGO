...
import { THEMED_PACKS, getSeasonalPack, getEventPack, isPackFree } from "./packs";
...
function swazAnswer(text,lang,profile){
  ...
  const A={
    fr:{
      howto:`C'est simple ${name} ! Chaque matin tu choisis 3 tâches principales (et des bonus si tu veux en faire plus), et le soir tu fais ton bilan : fait ou pas fait, sans jugement. Tu gagnes des Jolingos qui font grandir ton arbre et débloquent des Jokis. 🌱`,
    ...
    en:{
      howto:`It's simple ${name}! Each morning you pick 3 main tasks (plus bonus ones if you want to do more), each evening you review: done or not, no judgment. You earn Jolingos that grow your tree and unlock Jokis. 🌱`,
    ...
    es:{
      howto:`¡Es simple ${name}! Cada mañana eliges 3 tareas principales (y bonus si quieres hacer más), cada noche haces tu balance: hecho o no, sin juicios. Ganas Jolingos que hacen crecer tu árbol y desbloquean Jokis. 🌱`,
...
// ── SELECTION TÂCHES — 3 principales + bonus optionnels ────
...
function TaskSelection({lang,vibes,genre,onCommit,prefilled=[],isPrepTomorrow=false}){
  ...
  // Tâches planifiées via l'agenda pour aujourd'hui (proposées, non imposées)
  const plannedToday=isPrepTomorrow?[]:plannedForDay(s_.plannedTasks||{},todayKey()).map(t=>t.text);
  ...
  const isPremium=(load().isPremium===true);
    ...
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",justifyContent:"center"}}>
      <div style={{width:"100%",maxWidth:420,padding:"48px 20px 150px"}}>
        ...
        <div style={{fontFamily:"Inter,sans-serif",fontSize:13,color:C.muted,marginBottom:18}}>
          {lang==="fr"?"3 tâches principales, et des bonus si tu veux":lang==="es"?"3 tareas principales, y bonus si quieres":"3 main tasks, plus bonus if you like"}
        </div>
            ...
            {selected.map((s,i)=>{
              ...
              <div key={i}>
                {i===CORE_TASKS&&(
                  <div style={{margin:"14px 0 10px"}}>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      ...
                      <span style={{fontFamily:"Inter,sans-serif",fontSize:10,fontWeight:700,color:C.violet,letterSpacing:0.5,textTransform:"uppercase"}}>⭐ {lang==="fr"?"Tâches bonus":lang==="es"?"Tareas bonus":"Bonus tasks"}</span>
                      <div style={{flex:1,height:1,background:C.border}}/>
                    </div>
                    <div style={{fontFamily:"Inter,sans-serif",fontSize:11,color:C.muted,textAlign:"center",marginTop:6,lineHeight:1.4}}>
                      {lang==="fr"?`En plus de tes 3 tâches principales · ${PTS_BONUS} points chacune`:lang==="es"?`Además de tus 3 tareas principales · ${PTS_BONUS} puntos cada una`:`On top of your 3 main tasks · ${PTS_BONUS} points each`}
                    </div>
                  </div>
                )}
                <div style={{background:isBonus?"#FBFAFF":"white",borderRadius:13,padding:"13px 15px",marginBottom:8,display:"flex",alignItems:"center",gap:10,border:`2px solid ${isBonus?C.border:C.violet}`}}>
                  ...
                  <span style={{fontFamily:"Inter,sans-serif",fontSize:10,fontWeight:700,color:isBonus?C.muted:C.violet}}>{pts} {lang==="fr"?"pts":lang==="es"?"pts":"pts"}</span>
        ...
        {plannedToday.filter(text=>!isSelected(text)).length>0&&(
          <div style={{marginBottom:16}}>
            <div style={{fontFamily:"Inter,sans-serif",fontWeight:700,fontSize:12,color:C.violet,letterSpacing:1,textTransform:"uppercase",marginBottom:8,display:"flex",alignItems:"center",gap:6}}>
              📅 {lang==="fr"?"Prévu aujourd'hui":lang==="es"?"Previsto hoy":"Planned today"}
            </div>
            {plannedToday.filter(text=>!isSelected(text)).map((text,i)=>(
              <div key={i} onClick={()=>addTask({text,packId:"custom",isMine:myLib.includes(text),isResistance:false,carried:false,fromAI:false})}
                style={{background:`${C.violet}08`,borderRadius:13,padding:"12px 15px",marginBottom:8,display:"flex",alignItems:"center",gap:10,border:`1.5px solid ${C.violet}40`,cursor:atLimit?"not-allowed":"pointer",opacity:atLimit?0.5:1}}>
        ...
        <div style={{marginTop:8}}>
          ...
          {showPacks&&(()=>{
            ...
            const renderPack=(pack)=>{
              ...
              const locked=!isPackFree(pack)&&!isPremium;
              return(
                <div key={pack.id} style={{marginBottom:8}}>
                  <button onClick={()=>{if(locked){setShowPremium(true);}else{setOpenPack(open?null:pack.id);}}}
                    style={{width:"100%",padding:"12px 14px",border:`1.5px solid ${open?C.violet:C.border}`,borderRadius:12,background:open?"#F0F0FF":locked?"#FAFAFA":"white",display:"flex",alignItems:"center",gap:10,cursor:"pointer",opacity:locked?0.85:1}}>
                    <span style={{fontSize:20,filter:locked?"grayscale(0.4)":"none"}}>{pack.icon}</span>
                    <div style={{flex:1,textAlign:"left"}}>
                      <div style={{fontFamily:"Inter,sans-serif",fontWeight:700,fontSize:14,color:C.ink,display:"flex",alignItems:"center",gap:6}}>{pack.title[lang]}{locked&&<span style={{fontSize:10,fontWeight:700,color:C.yellow,background:C.ink,borderRadius:5,padding:"1px 6px"}}>PREMIUM</span>}</div>
                      <div style={{fontFamily:"Inter,sans-serif",fontSize:11,color:C.muted}}>{pack.desc[lang]}</div>
                    </div>
                    <span style={{color:C.muted}}>{locked?"🔒":(open?"▲":"▼")}</span>
                  </button>
                  {open&&!locked&&(
      ...
      {showPremium&&(
        <div onClick={()=>setShowPremium(false)} style={{position:"fixed",inset:0,background:"rgba(26,26,46,.7)",backdropFilter:"blur(5px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:700,padding:24}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"white",borderRadius:24,padding:"28px 24px",maxWidth:340,width:"100%",textAlign:"center"}}>
            <div style={{fontSize:44,marginBottom:8}}>🔒</div>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:28,color:C.ink}}>{lang==="fr"?"Pack Premium":lang==="es"?"Pack Premium":"Premium Pack"}</div>
            <div style={{fontFamily:"Inter,sans-serif",fontSize:14,color:C.muted,lineHeight:1.6,margin:"10px 0 20px"}}>
              {lang==="fr"?"Les packs Bien-être, Productivité, Fêtes et Relations font partie de Jolingo Premium. Les 4 packs de saison restent gratuits pour toi !":lang==="es"?"Los packs de Bienestar, Productividad, Fiestas y Relaciones son parte de Jolingo Premium. ¡Los 4 packs de temporada siguen gratis!":"The Wellbeing, Productivity, Celebrations and Relationships packs are part of Jolingo Premium. The 4 seasonal packs stay free for you!"}
            </div>
            <div style={{background:C.bg,borderRadius:12,padding:"12px 14px",marginBottom:18,fontFamily:"Inter,sans-serif",fontSize:12,color:C.muted}}>
              {lang==="fr"?"💛 Bientôt disponible":lang==="es"?"💛 Próximamente":"💛 Coming soon"}
            </div>
            <button onClick={()=>setShowPremium(false)} style={{width:"100%",padding:"14px",border:"none",borderRadius:13,background:C.violet,color:"white",fontFamily:"Inter,sans-serif",fontWeight:700,fontSize:15,cursor:"pointer"}}>
              {lang==="fr"?"Compris":lang==="es"?"Entendido":"Got it"}
            </button>
          </div>
        </div>
      )}
...
// Statistiques de reporting global (records, évolution, régularité, répartition)
function reportStats(history,bestStreakStored,totalDone){
  const entries=Object.entries(history||{});
  // Record de série : dérivé de l'historique (jours consécutifs actifs) + valeur stockée
  const activeKeys=entries.filter(([,v])=>v.done>0||v.total>0).map(([k])=>k).sort();
  let bestFromHist=0,run=0,prev=null;
  activeKeys.forEach(k=>{
    ...
    bestFromHist=Math.max(bestFromHist,run);
    prev=d;
  });
  const bestStreak=Math.max(bestStreakStored||0,bestFromHist);
  // Meilleur mois (en Jolingos)
  const byMonth={};
  entries.forEach(([k,v])=>{const mk=k.slice(0,7);byMonth[mk]=(byMonth[mk]||0)+(v.pts||0);});
  let bestMonth=null,bestMonthVal=0;
  Object.entries(byMonth).forEach(([mk,val])=>{if(val>bestMonthVal){bestMonthVal=val;bestMonth=mk;}});
  // Évolution : 6 derniers mois (tâches faites)
  const now=new Date();
  const evo=[];
  for(let i=5;i>=0;i--){
    ...
    let tasks=0;
    entries.forEach(([k,v])=>{if(k.startsWith(mk))tasks+=v.done||0;});
    evo.push({mk,label:["jan","fév","mar","avr","mai","juin","juil","aoû","sep","oct","nov","déc"][d.getMonth()],tasks});
  }
  // Régularité : jours actifs sur les 30 derniers jours
  let active30=0;
  for(let i=0;i<30;i++){
    const d=new Date(now);d.setDate(d.getDate()-i);
    const k=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
    if(history[k]&&(history[k].done>0))active30++;
  }
  // Répartition des types de tâches (à partir des cats stockées)
  const catCount={};
  entries.forEach(([,v])=>{(v.cats||[]).forEach(c=>{catCount[c]=(catCount[c]||0)+1;});});
  return{bestStreak,bestMonth,bestMonthVal,evo,active30,catCount};
}

// ── PLANIFICATION (agenda) — tâches déposées sur des dates futures ──
// Stockées dans state.plannedTasks = { "YYYY-MM-DD": [{text, repeat}], ... }
// repeat: null | "daily" | "weekly" (même jour de semaine)
function loadPlanned(){ return (load().plannedTasks)||{}; }
function savePlanned(pt){ const s=load(); save({...s,plannedTasks:pt}); }
function dateKeyOf(d){ return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`; }
// Renvoie les tâches planifiées pour un jour donné (en tenant compte des récurrences)
function plannedForDay(pt,dateKey){
  const direct=(pt[dateKey]||[]).slice();
  const target=new Date(dateKey);
  const targetDow=target.getDay();
  // Parcourir les récurrences définies sur des dates antérieures
  Object.entries(pt).forEach(([k,list])=>{
    if(k===dateKey) return;
    const src=new Date(k);
    if(src>target) return; // une récurrence ne s'applique qu'à partir de sa date de création
    list.forEach(task=>{
      if(!task.repeat) return;
      if(task.repeat==="daily") direct.push({...task,recurred:true});
      else if(task.repeat==="weekly"&&src.getDay()===targetDow) direct.push({...task,recurred:true});
    });
  });
  return direct;
}
...
// ── AGENDA / CALENDRIER — planifier des tâches à l'avance ──
function CalendarTab({lang,genre,vibes,onRefreshHome}){
  const [refresh,setRefresh]=useState(0);
  const [showMonth,setShowMonth]=useState(false);
  const [anchor,setAnchor]=useState(()=>jolingoDate()); // date de référence (semaine/mois affiché)
  const [selDate,setSelDate]=useState(()=>jolingoDate());
  const [adding,setAdding]=useState(false);
  const [newText,setNewText]=useState("");
  const [repeat,setRepeat]=useState(null);
  const pt=loadPlanned();
  const todayK=dateKeyOf(jolingoDate());
  const selK=dateKeyOf(selDate);
  const MONTHS=lang==="fr"?["Janvier","Février","Mars","Avril","Mai","Juin","Juillet","Août","Septembre","Octobre","Novembre","Décembre"]:lang==="es"?["Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"]:["January","February","March","April","May","June","July","August","September","October","November","December"];
  const DOW=lang==="fr"?["L","M","M","J","V","S","D"]:lang==="es"?["L","M","X","J","V","S","D"]:["M","T","W","T","F","S","S"];

  // Semaine courante (lundi -> dimanche) autour de anchor
  function weekDays(){
  ...
  function monthCells(){
    ...
    for(let d=1;d<=nb;d++)cells.push(new Date(y,m,d));
    return cells;
  }
  function shiftWeek(dir){const d=new Date(anchor);d.setDate(d.getDate()+dir*7);setAnchor(d);}
  function shiftMonth(dir){const d=new Date(anchor);d.setMonth(d.getMonth()+dir);setAnchor(d);}

  function addPlanned(){
    if(!newText.trim())return;
    const cur=loadPlanned();
    const list=(cur[selK]||[]).slice();
    list.push({text:newText.trim(),repeat});
    savePlanned({...cur,[selK]:list});
    setNewText("");setRepeat(null);setAdding(false);setRefresh(r=>r+1);
    if(navigator.vibrate)navigator.vibrate(15);
  }
  function removePlanned(idx){
    const cur=loadPlanned();
    const list=(cur[selK]||[]).slice();
    list.splice(idx,1);
    savePlanned({...cur,[selK]:list});
  ...
  const dayTasks=plannedForDay(loadPlanned(),selK);
  const repeatLabel=(r)=>r==="daily"?(lang==="fr"?"chaque jour":lang==="es"?"cada día":"daily"):r==="weekly"?(lang==="fr"?"chaque semaine":lang==="es"?"cada semana":"weekly"):"";
  const selLabel=(()=>{
    const opts={weekday:"long",day:"numeric",month:"long"};
    const loc=lang==="fr"?"fr-FR":lang==="es"?"es-ES":"en-GB";
    let s=selDate.toLocaleDateString(loc,opts);
    s=s.charAt(0).toUpperCase()+s.slice(1);
    if(selK===todayK)s+=lang==="fr"?" · Aujourd'hui":lang==="es"?" · Hoy":" · Today";
    return s;
  })();

  return(
    <>
      <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:34,color:C.ink,marginBottom:6}}>📅 {lang==="fr"?"AGENDA":lang==="es"?"AGENDA":"CALENDAR"}</div>
      <div style={{fontFamily:"Inter,sans-serif",fontSize:13,color:C.muted,marginBottom:18}}>{lang==="fr"?"Planifie tes tâches à l'avance":lang==="es"?"Planifica tus tareas con antelación":"Plan your tasks ahead"}</div>

      {/* Sélecteur semaine / mois */}
      <div style={{background:"white",borderRadius:18,padding:"16px 14px",marginBottom:14,boxShadow:`0 4px 16px ${C.violet}0D`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          <span style={{color:C.muted,fontSize:20,cursor:"pointer",padding:"0 8px"}} onClick={()=>showMonth?shiftMonth(-1):shiftWeek(-1)}>‹</span>
          <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:19,color:C.ink}}>{MONTHS[anchor.getMonth()]} {anchor.getFullYear()}</span>
          <span style={{color:C.muted,fontSize:20,cursor:"pointer",padding:"0 8px"}} onClick={()=>showMonth?shiftMonth(1):shiftWeek(1)}>›</span>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2,marginBottom:6}}>
          {DOW.map((d,i)=><div key={i} style={{textAlign:"center",fontFamily:"Inter,sans-serif",fontSize:10,fontWeight:700,color:C.muted}}>{d}</div>)}
        </div>
        {!showMonth?(
          <div style={{display:"flex",justifyContent:"space-between",gap:4}}>
            {weekDays().map((d,i)=>{
              const k=dateKeyOf(d);const isSel=k===selK;const isToday=k===todayK;
              const has=plannedForDay(pt,k).length;
              return(
                <div key={i} onClick={()=>setSelDate(d)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4,padding:"8px 0",borderRadius:12,cursor:"pointer",background:isSel?C.violet:"transparent"}}>
                  <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:18,color:isSel?"white":isToday?C.violet:C.ink}}>{d.getDate()}</span>
                  {has?<div style={{width:4,height:4,borderRadius:"50%",background:isSel?"white":C.violet}}/>:<div style={{height:4}}/>}
                </div>
              );
            })}
          </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2}}>
            {monthCells().map((d,i)=>{
              ...
              const has=plannedForDay(pt,k).length;
              return(
                <div key={i} onClick={()=>setSelDate(d)} style={{aspectRatio:"1",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",borderRadius:9,cursor:"pointer",background:isSel?C.violet:"transparent"}}>
                  <span style={{fontFamily:"Inter,sans-serif",fontSize:13,fontWeight:isToday?800:500,color:isSel?"white":isToday?C.violet:C.ink}}>{d.getDate()}</span>
                  {has?<div style={{width:3.5,height:3.5,borderRadius:"50%",background:isSel?"white":C.violet,marginTop:1}}/>:null}
                </div>
              );
            })}
          </div>
        ...
        <div style={{display:"flex",justifyContent:"center",marginTop:10}}>
          <button onClick={()=>setShowMonth(!showMonth)} style={{fontFamily:"Inter,sans-serif",fontSize:12,color:C.violet,fontWeight:700,background:`${C.violet}12`,border:"none",borderRadius:8,padding:"6px 12px",cursor:"pointer"}}>
            {showMonth?(lang==="fr"?"Réduire ▴":lang==="es"?"Reducir ▴":"Collapse ▴"):(lang==="fr"?"Voir le mois ▾":lang==="es"?"Ver el mes ▾":"See month ▾")}
          </button>
        </div>
      </div>

      {/* Tâches du jour sélectionné */}
      <div style={{background:"white",borderRadius:16,padding:"16px 18px",border:`1px solid ${C.border}`}}>
        <div style={{fontFamily:"Inter,sans-serif",fontSize:12,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:.5,marginBottom:12}}>{selLabel}</div>
        {dayTasks.map((task,i)=>(
          <div key={i} style={{background:C.bg,borderRadius:11,padding:"11px 13px",marginBottom:6,display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:13}}>{task.repeat||task.recurred?"🔁":"📅"}</span>
            <span style={{fontFamily:"Inter,sans-serif",fontSize:14,color:C.ink,flex:1}}>{task.text}{task.repeat&&<span style={{fontSize:11,color:C.muted,marginLeft:6}}>· {repeatLabel(task.repeat)}</span>}</span>
            {!task.recurred&&<button onClick={()=>removePlanned(i)} style={{background:"none",border:"none",cursor:"pointer",color:C.muted,fontSize:18}}>×</button>}
          </div>
        ))}
        {dayTasks.length===0&&!adding&&<div style={{fontFamily:"Inter,sans-serif",fontSize:13,color:C.muted,fontStyle:"italic",marginBottom:10}}>{lang==="fr"?"Rien de prévu ce jour-là.":lang==="es"?"Nada previsto ese día.":"Nothing planned that day."}</div>}

        {adding?(
          <div style={{marginTop:8}}>
            <input autoFocus value={newText} onChange={e=>setNewText(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addPlanned()}
              placeholder={lang==="fr"?"Ta tâche…":lang==="es"?"Tu tarea…":"Your task…"}
              style={{width:"100%",padding:"12px 14px",border:`2px solid ${C.violet}`,borderRadius:11,fontFamily:"Inter,sans-serif",fontSize:14,outline:"none",marginBottom:10}}/>
            <div style={{display:"flex",gap:6,marginBottom:10}}>
              {[[null,lang==="fr"?"Une fois":lang==="es"?"Una vez":"Once"],["daily",lang==="fr"?"Chaque jour":lang==="es"?"Cada día":"Daily"],["weekly",lang==="fr"?"Chaque semaine":lang==="es"?"Cada semana":"Weekly"]].map(([val,lab])=>(
                <button key={String(val)} onClick={()=>setRepeat(val)} style={{flex:1,padding:"9px 4px",border:`1.5px solid ${repeat===val?C.violet:C.border}`,borderRadius:10,background:repeat===val?"#F0F0FF":"white",color:repeat===val?C.violet:C.muted,fontFamily:"Inter,sans-serif",fontWeight:700,fontSize:11.5,cursor:"pointer"}}>{lab}</button>
              ))}
            </div>
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>{setAdding(false);setNewText("");setRepeat(null);}} style={{flex:1,padding:"12px",border:`1.5px solid ${C.border}`,borderRadius:11,background:"white",color:C.muted,fontFamily:"Inter,sans-serif",fontWeight:700,fontSize:14,cursor:"pointer"}}>{lang==="fr"?"Annuler":lang==="es"?"Cancelar":"Cancel"}</button>
              <button onClick={addPlanned} style={{flex:2,padding:"12px",border:"none",borderRadius:11,background:C.violet,color:"white",fontFamily:"Inter,sans-serif",fontWeight:700,fontSize:14,cursor:"pointer"}}>{lang==="fr"?"Planifier":lang==="es"?"Planificar":"Plan it"}</button>
            </div>
          </div>
        ):(
          <button onClick={()=>setAdding(true)} style={{width:"100%",padding:"12px",border:`2px dashed ${C.violet}`,borderRadius:11,background:"transparent",color:C.violet,fontFamily:"Inter,sans-serif",fontWeight:700,fontSize:14,cursor:"pointer",marginTop:4}}>+ {lang==="fr"?"Planifier une tâche":lang==="es"?"Planificar una tarea":"Plan a task"}</button>
        )}
      </div>
      <div style={{fontFamily:"Inter,sans-serif",fontSize:11.5,color:C.muted,textAlign:"center",marginTop:14,lineHeight:1.5,padding:"0 10px"}}>
        {lang==="fr"?"Le jour venu, tes tâches planifiées te seront proposées au moment de choisir ta journée. 🌱":lang==="es"?"El día llegado, tus tareas planificadas te serán propuestas al elegir tu día. 🌱":"When the day comes, your planned tasks will be suggested as you choose your day. 🌱"}
      </div>
    </>
  );
}
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    if(s.activeKey!==tk){
      if(s.tomorrowTasks?.length>0&&s.tomorrowPreparedFor===tk){
        ...
        const best1=Math.max(s.bestStreak||0,newStreak);
        save({...s,activeKey:tk,selected:s.tomorrowTasks,checkin:{},eveningDone:false,tomorrowTasks:[],tomorrowPreparedFor:null,selectedDays:selDays2,streak:newStreak,bestStreak:best1});
        setScreen("evening");
  ...
  function submitEvening(){
    ...
    const cats=selected.map(t=>{
      if(t.isMine||t.packId==="custom") return "perso";
      const pk=THEMED_PACKS.find(p=>p.id===t.packId);
      if(pk) return pk.family||"core";
      return "vibe"; // tâches des univers de base
    });
    const newH={...prevH,[tk]:{done,total,pts,isPerfect,taskTexts:selected.map(t=>t.text),cats}};
  ...
  if(screen==="select") return <TaskSelection lang={lang} vibes={vibes} genre={genre} onCommit={tasks=>{
    ...
    save({...s,activeKey:tk,selected:tasks,checkin:{},eveningDone:false,carriedTasks:[],selectedDays:selDays,streak:newStreak,reachedMilestones:reached,bestStreak:Math.max(s.bestStreak||0,newStreak)});
  ...
  if(screen==="prepTomorrow"){
    const s=load();
    const raw=(s.tomorrowPreparedFor===tomorrowKey()&&s.tomorrowTasks?.length>0)?s.tomorrowTasks:[];
    // Les tâches peuvent être des objets {text,...} ou des chaînes : on normalise en objets
    const pre=raw.map(tt=>typeof tt==="string"?{text:tt}:tt);
      ...
      <div style={{width:"100%",maxWidth:420,padding:"48px 20px 100px"}}>
        ...
        {tab==="home"&&screen==="home"&&(
          <>
            ...
            <div className="fi" style={{display:"flex",gap:10,marginBottom:16,marginTop:streak>0?0:16}}>
              {[
                {l:"✅",v:totalDone,cap:lang==="fr"?"tâches faites":lang==="es"?"tareas hechas":"tasks done"},
                {l:"⭐",v:perfectDays,cap:lang==="fr"?"jours parfaits":lang==="es"?"días perfectos":"perfect days"},
                {l:"coin",v:`${monthlyJolingos[monthKey()]||0}`,cap:lang==="fr"?"ce mois-ci":lang==="es"?"este mes":"this month"},
              ].map((s,i)=>(
                <div key={i} style={{flex:1,background:"white",borderRadius:14,padding:"12px 8px",textAlign:"center"}}>
                  ...
                  <div style={{fontFamily:"Inter,sans-serif",fontSize:9.5,color:C.muted,marginTop:2,lineHeight:1.2}}>{s.cap}</div>
                </div>
              ))}
            </div>
        ...
        {tab==="stats"&&(
          <>
            ...
            {/* ── TABLEAU DE BORD ── */}
            {(()=>{
              const rs=reportStats(history,load().bestStreak,totalDone);
              const catLabels={
                perso:{fr:"Perso",en:"Personal",es:"Personal"},
                vibe:{fr:"Univers",en:"Universes",es:"Universos"},
                core:{fr:"Saisons",en:"Seasons",es:"Estaciones"},
                wellbeing:{fr:"Bien-être",en:"Wellbeing",es:"Bienestar"},
                productivity:{fr:"Productivité",en:"Productivity",es:"Productividad"},
                festive:{fr:"Fêtes",en:"Celebrations",es:"Fiestas"},
                relations:{fr:"Relations",en:"Relations",es:"Relaciones"},
              };
              const catColors={perso:C.violet,vibe:"#A99BF5",core:"#22C55E",wellbeing:"#4ECDC4",productivity:"#F59E0B",festive:"#FF6B9D",relations:"#FF8C69"};
              const totalCats=Object.values(rs.catCount).reduce((a,b)=>a+b,0);
              const maxEvo=Math.max(1,...rs.evo.map(e=>e.tasks));
              return(
                <>
                  {/* Records */}
                  <div style={{display:"flex",gap:8,marginBottom:14}}>
                    {[
                      {ic:"🔥",v:rs.bestStreak,l:lang==="fr"?"record série":lang==="es"?"récord racha":"best streak"},
                      {ic:"🏆",v:rs.bestMonthVal,l:lang==="fr"?"meilleur mois":lang==="es"?"mejor mes":"best month"},
                      {ic:"✅",v:totalDone,l:lang==="fr"?"total tâches":lang==="es"?"total tareas":"total tasks"},
                    ].map((r,i)=>(
                      <div key={i} style={{flex:1,background:"white",borderRadius:14,padding:"14px 8px",textAlign:"center"}}>
                        <div style={{fontSize:20}}>{r.ic}</div>
                        <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:26,color:C.ink,marginTop:2}}>{r.v}</div>
                        <div style={{fontFamily:"Inter,sans-serif",fontSize:9.5,color:C.muted,marginTop:1}}>{r.l}</div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div style={{background:"white",borderRadius:18,padding:"18px 20px",marginBottom:14}}>
                    <div style={{fontFamily:"Inter,sans-serif",fontWeight:700,fontSize:12,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:16}}>📈 {lang==="fr"?"Évolution (6 mois)":lang==="es"?"Evolución (6 meses)":"Evolution (6 months)"}</div>
                    <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",gap:8,height:80}}>
                      {rs.evo.map((e,i)=>(
                        <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:5}}>
                          <span style={{fontFamily:"Inter,sans-serif",fontSize:10,color:C.ink,fontWeight:700}}>{e.tasks}</span>
                          <div style={{width:"100%",maxWidth:30,height:`${Math.max(4,(e.tasks/maxEvo)*56)}px`,borderRadius:6,background:i===rs.evo.length-1?C.violet:"#D8D3F5",transition:"height .3s"}}/>
                          <span style={{fontFamily:"Inter,sans-serif",fontSize:9,color:C.muted}}>{e.label}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div style={{background:"white",borderRadius:18,padding:"18px 20px",marginBottom:14}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                      <div style={{fontFamily:"Inter,sans-serif",fontWeight:700,fontSize:12,color:C.muted,letterSpacing:1,textTransform:"uppercase"}}>⚡ {lang==="fr"?"Régularité (30 j)":lang==="es"?"Regularidad (30 d)":"Consistency (30 d)"}</div>
                      <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,color:C.violet}}>{rs.active30}/30</div>
                    </div>
                    <div style={{height:10,background:C.bg,borderRadius:5,overflow:"hidden"}}>
                      <div style={{height:"100%",width:`${(rs.active30/30)*100}%`,background:`linear-gradient(90deg,${C.violetL},${C.violet})`,borderRadius:5,transition:"width .5s"}}/>
                    </div>
                    <div style={{fontFamily:"Inter,sans-serif",fontSize:11,color:C.muted,marginTop:8}}>
                      {rs.active30>=20?(lang==="fr"?"Superbe régularité ! 🌟":lang==="es"?"¡Excelente regularidad! 🌟":"Great consistency! 🌟"):rs.active30>=10?(lang==="fr"?"Beau rythme, continue 💪":lang==="es"?"Buen ritmo, sigue 💪":"Good pace, keep going 💪"):(lang==="fr"?"Chaque jour compte 🌱":lang==="es"?"Cada día cuenta 🌱":"Every day counts 🌱")}
                    </div>
                  </div>
                  ...
                  {totalCats>0&&(
                    <div style={{background:"white",borderRadius:18,padding:"18px 20px",marginBottom:14}}>
                      <div style={{fontFamily:"Inter,sans-serif",fontWeight:700,fontSize:12,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:14}}>🎯 {lang==="fr"?"Tes types de tâches":lang==="es"?"Tus tipos de tareas":"Your task types"}</div>
                      {/* barre empilée */}
                      <div style={{display:"flex",height:12,borderRadius:6,overflow:"hidden",marginBottom:12}}>
                        {Object.entries(rs.catCount).sort(([,a],[,b])=>b-a).map(([cat,n])=>(
                          <div key={cat} style={{width:`${(n/totalCats)*100}%`,background:catColors[cat]||C.muted}}/>
                        ))}
                      </div>
                      <div style={{display:"flex",flexWrap:"wrap",gap:"6px 14px"}}>
                        {Object.entries(rs.catCount).sort(([,a],[,b])=>b-a).map(([cat,n])=>(
                          <div key={cat} style={{display:"flex",alignItems:"center",gap:5}}>
                            <div style={{width:9,height:9,borderRadius:3,background:catColors[cat]||C.muted}}/>
                            <span style={{fontFamily:"Inter,sans-serif",fontSize:11,color:C.ink}}>{(catLabels[cat]||{})[lang]||cat}</span>
                            <span style={{fontFamily:"Inter,sans-serif",fontSize:11,color:C.muted}}>{Math.round((n/totalCats)*100)}%</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div style={{fontFamily:"Inter,sans-serif",fontWeight:700,fontSize:12,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:12,marginTop:4}}>{lang==="fr"?"Par mois":lang==="es"?"Por mes":"By month"}</div>
                </>
              );
            })()}
            <div style={{display:"flex",gap:8,marginBottom:20,overflowX:"auto",paddingBottom:4}}>
              {allMonths.map(mk=>(
                <button key={mk} onClick={()=>setSelectedMonth(mk)}
                  ...
                  {(()=>{const[y,m]=mk.split("-");return t.months[parseInt(m)-1]+" "+y;})()}
                </button>
              ))}
            </div>
        ...
        {tab==="calendar"&&(
          <CalendarTab lang={lang} genre={genre} vibes={vibes}/>
        )}
      </div>
        ...
        <div className="bottom-nav">
          <div className="bottom-inner">
            {[
              ...
              {id:"calendar",emoji:"📅",label:lang==="fr"?"Agenda":lang==="es"?"Agenda":"Calendar"},
