...
async function generateSwazMessage(lang,context,profile){
  // context: streak | capture | slump | welcome
  try{
    const name=profile?.name||"";
    const sysMap={
      fr:`Tu es Swaz, un coach bienveillant dans l'app Jolingo. Tu parles à ${name||"l'utilisateur"} avec tutoiement chaleureux, comme un ami proche. Règles ABSOLUES: toujours doux et encourageant, JAMAIS culpabilisant. Refuse tout ce qui est malsain, dangereux ou non éthique (auto-dévalorisation, objectifs extrêmes, troubles alimentaires). 1 phrase max, 1 emoji max. Réponds UNIQUEMENT par le message, rien d'autre.`,
      en:`You are Swaz, a caring coach in the Jolingo app. You speak to ${name||"the user"} warmly, like a close friend. ABSOLUTE rules: always gentle and encouraging, NEVER guilt-inducing. Refuse anything unhealthy, dangerous or unethical. 1 sentence max, 1 emoji max. Reply ONLY with the message.`,
      es:`Eres Swaz, un coach amable en la app Jolingo. Hablas a ${name||"el usuario"} con calidez, como un amigo cercano. Reglas ABSOLUTAS: siempre suave y alentador, NUNCA culpabilizando. Rechaza todo lo malsano, peligroso o poco ético. 1 frase máx, 1 emoji máx. Responde SOLO con el mensaje.`,
    };
    const ctxMap={
      fr:{streak:"L'utilisateur vient de prolonger sa série. Félicite-le.",capture:"L'utilisateur vient de capturer un nouveau Joki/Énergie. Célèbre.",slump:"L'utilisateur a ralenti ces derniers jours. Encourage-le doucement, sans culpabiliser.",welcome:"Accueille l'utilisateur pour sa journée."},
      en:{streak:"The user just extended their streak. Congratulate them.",capture:"The user just captured a new Joki/Energy. Celebrate.",slump:"The user slowed down recently. Gently encourage, no guilt.",welcome:"Welcome the user for their day."},
      es:{streak:"El usuario extendió su racha. Felicítalo.",capture:"El usuario capturó un nuevo Joki/Energía. Celebra.",slump:"El usuario bajó el ritmo. Anima con suavidad, sin culpa.",welcome:"Da la bienvenida al usuario."},
    };
    const sys=sysMap[lang]||sysMap.fr;
    const usr=(ctxMap[lang]||ctxMap.fr)[context]||"";
    const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:80,system:sys,messages:[{role:"user",content:usr}]})});
    const data=await res.json();
    const msg=data.content?.map(i=>i.text||"").join("").trim();
...
async function askSwaz(lang,question,profile,history){
  try{
    const name=profile?.name||"";
    const sysMap={
      fr:`Tu es Swaz, le coach bienveillant de l'app Jolingo. Jolingo est une app de productivité douce: chaque matin l'utilisateur choisit jusqu'à 3 tâches, le soir il fait son bilan (fait/pas fait). Il gagne des Jolingos (points): 10 par tâche, 15 pour un défi "résistance" (résister à une mauvaise habitude), 20 pour une journée parfaite, 5 par jour de série. En accumulant des Jolingos il capture des Jokis (personnages) et leurs Énergies de 20 pays. Son arbre "Mon Jodama" grandit. Tu parles à ${name||"l'utilisateur"} avec tutoiement chaleureux, comme un ami proche. RÈGLES ABSOLUES: toujours bienveillant, jamais culpabilisant. Tu refuses poliment tout ce qui est malsain, dangereux, illégal ou non éthique (auto-dévalorisation, objectifs extrêmes, troubles alimentaires, etc.) et tu rediriges vers du positif. Si on te demande des tâches, propose des idées simples, positives, réalisables. Réponses courtes (2-3 phrases max), chaleureuses, avec parcimonie d'emojis.`,
      en:`You are Swaz, the caring coach of the Jolingo app. Jolingo is a gentle productivity app: each morning the user picks up to 3 tasks, each evening they review (done/not done). They earn Jolingos (points): 10 per task, 15 for a "resistance" challenge, 20 for a perfect day, 5 per streak day. Accumulating Jolingos captures Jokis (characters) and their Energies from 20 countries. Their tree "My Jodama" grows. You speak to ${name||"the user"} warmly, like a close friend. ABSOLUTE rules: always caring, never guilt-inducing. Politely refuse anything unhealthy, dangerous, illegal or unethical and redirect to positivity. If asked for tasks, suggest simple, positive, doable ideas. Short replies (2-3 sentences max), warm, sparing emojis.`,
      es:`Eres Swaz, el coach amable de la app Jolingo. Jolingo es una app de productividad suave: cada mañana el usuario elige hasta 3 tareas, cada noche hace su balance (hecho/no hecho). Gana Jolingos (puntos): 10 por tarea, 15 por un reto de "resistencia", 20 por un día perfecto, 5 por día de racha. Acumulando Jolingos captura Jokis (personajes) y sus Energías de 20 países. Su árbol "Mi Jodama" crece. Hablas a ${name||"el usuario"} con calidez, como un amigo. Reglas ABSOLUTAS: siempre amable, nunca culpabilizando. Rechaza con cortesía lo malsano, peligroso, ilegal o poco ético y redirige a lo positivo. Si piden tareas, sugiere ideas simples, positivas, realizables. Respuestas cortas (2-3 frases máx), cálidas, pocos emojis.`,
    };
    const sys=sysMap[lang]||sysMap.fr;
    const msgs=[...(history||[]).slice(-6),{role:"user",content:question}];
    const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:300,system:sys,messages:msgs})});
    const data=await res.json();
    const msg=data.content?.map(i=>i.text||"").join("").trim();
    return msg||swazFallback(lang,"welcome");
  }catch(e){
    return lang==="fr"?"Oups, je n'ai pas pu répondre là. Réessaie dans un instant ! 💛":lang==="es"?"Ups, no pude responder. ¡Inténtalo de nuevo! 💛":"Oops, I couldn't reply. Try again in a moment! 💛";
  }
}
...
function TaskSelection({lang,vibes,genre,onCommit,prefilled=[]}){
  ...
  async function generateFromAI(){
    ...
    }catch(e){flash("error",lang==="fr"?"Oups, réessaie !":"Oops, try again!");}
    setAiLoading(false);
  }
