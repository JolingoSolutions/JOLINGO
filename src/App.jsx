        ...
        {w.days.map((d,i)=>{
          ...
          return(
            <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
              <div style={{width:"100%",maxWidth:26,height:h,borderRadius:6,background:d.isPerfect?C.violet:d.done>0?C.violetL||"#9B95FF":C.border,transition:"height .3s"}}/>
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    if((s.streak||0)>0&&!selDays[yk]&&!selDays[tk]){
      ...
      <div style={{width:"100%",maxWidth:420,padding:"48px 20px 100px"}}>
        ...
        {tab==="home"&&screen==="home"&&(
          <>
            ...
            {/* Récap de la semaine */}
            <div className="fi" style={{animationDelay:".24s"}}>
                  ...
                  {selected.map((task,i)=>{
                    ...
                    return(
                      <div key={i} onClick={()=>toggleTaskDone(i)} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 0",borderBottom:i<selected.length-1?`1px solid ${C.border}`:"none",cursor:"pointer"}}>
                        ...
                        <span style={{fontFamily:"Inter,sans-serif",fontSize:11,color:isDone?C.green:C.muted,fontWeight:isDone?700:400,display:"inline-flex",alignItems:"center",gap:2}}>{task.carried?PTS_CARRIED:(task.isResistance?PTS_RESISTANCE:PTS_TASK)}<JolingoCoin size={13} gain={isDone}/></span>
                      </div>
                    );
                  })}
            ...
            {todayDone&&(
              <div style={{background:"white",borderRadius:20,padding:"28px 20px",textAlign:"center",border:`2px solid ${C.green}30`}}>
                ...
                {tomorrowTasks.length===0?(
                  ...
                  <div style={{marginTop:18,background:"#F0FDF4",borderRadius:12,padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"center",gap:8,border:`1px solid ${C.green}30`}}>
                    <span style={{fontSize:18}}>✓</span>
                    <span style={{fontFamily:"Inter,sans-serif",fontWeight:700,fontSize:13,color:C.green}}>{lang==="fr"?`Demain est prêt (${tomorrowTasks.length} tâches)`:lang==="es"?`Mañana está listo (${tomorrowTasks.length})`:`Tomorrow is ready (${tomorrowTasks.length})`}</span>
                  </div>
                )}
              </div>
            )}
            ...
            <div style={{textAlign:"left",marginTop:24}}>
              {selected.map((task,i)=>(
                <div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",background:"white",borderRadius:12,marginBottom:8}}>
                  ...
                  {checkin[i]===true&&<span style={{fontFamily:"Inter,sans-serif",fontSize:11,color:C.violet,fontWeight:700,display:"inline-flex",alignItems:"center",gap:2}}>+{task.carried?PTS_CARRIED:(task.isResistance?PTS_RESISTANCE:PTS_TASK)}<JolingoCoin size={13}/></span>}
                </div>
              ))}
            </div>
